    ...
    internal class Program2
    {
        ...
        public static void Execute()
        {
            ...
            try
            {
                ...
                List<Locale2> destinations = ReadDestinationsFromCSV("C:\\Users\\user\\Desktop\\최종프로젝트\\API\\경로 탐색 최신\\sleep.csv");
        ...
        private static void SaveToFile(Locale2 start, Locale2 destination, double distance, string distanceUnit)
        {
            try
            {
                string filePath = "C:\\Users\\user\\Desktop\\최종프로젝트\\API\\경로 탐색 최신\\closest_destination.txt";
                using (StreamWriter sw = File.CreateText(filePath))
                {
                    sw.WriteLine(start.Pname);
                    sw.WriteLine(start.Lat); // 천안역 위도
                    sw.WriteLine(start.Lng); // 천안역 경도
                    sw.WriteLine(destination.Pname);
