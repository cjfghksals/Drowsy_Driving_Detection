    ...
    internal class Program
    {
        ...
        {        
            ...
            try
            {
                ...
                List<Locale> destinations = ReadDestinationsFromCSV("C:\\Users\\user\\Desktop\\최종프로젝트\\API\\경로 탐색\\finaltest.csv");

                Locale start = new Locale("천안역", 127.14682487, 36.80898535, 0);

                List<(Locale, double)> distances = new List<(Locale, double)>();
        ...
        // CSV 파일에서 목적지 정보 읽어오기
        private static List<Locale> ReadDestinationsFromCSV(string filePath)
        {
            List<Locale> destinations = new List<Locale>();
                ...
                using (StreamReader sr = new StreamReader(filePath))
                {
                    while (!sr.EndOfStream)
                    {
                        ...
                        if (data.Length >= 3)
                        {
                            ...
                            {
                                destinations.Add(new Locale(pname, lng, lat, 0));
                            }
                            else
                            {
        ...
        // txt 파일로 저장
        private static void SaveToFile(Locale start, Locale destination, double distance, string distanceUnit)
        {
            try
            {
        ...
        // 경로 탐색 TMap API
        private static double SearchRoute(Locale start, Locale end)
        {
            try
            {
