...
if __name__ == '__main__':
    ...
    try:
        ...
        mid_point = width // 2
        threshold = 50  # 차선 이탈 감지를 위한 기준선 설정
        ...
        while True:
            ...
            frame=cv2.resize(frame,(0,0),fx=0.5,fy=0.5)
            isRed,g_cnt,r_cnt=detectTrafficLight(frame,isRed,g_cnt,r_cnt)

            # 프레임을 처리하고 차선을 검출
            processed_frame = process_frame(frame, mid_point, threshold)
            cv2.imshow('Traffic Light and Lane Detection', processed_frame)
